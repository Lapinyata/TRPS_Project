...
const buttonAuthReg = document.querySelector('.button-auth-reg'); // Получаю кнопку "Регистрация" в окне авторизации
const closeReg = document.querySelector('.close-reg'); // Получаю кнопку закрытия окна регистрации
const regForm = document.querySelector('#RegForm'); // Получаю форму регистрации в модальном окне регистрации
...
function notAuthorized() { // Функция, которая выполняется, когда пользователь неавторизован
  ...
  function logIn(event) { // Функция, авторизующая пользователя
    ...
    if(event.submitter.classList.contains('button-login')) {
      ...
      function regIn(event){ // Функция, регистрирующая пользователя
        event.preventDefault(); // отмена стандартного поведения браузера (чтобы на submit страница не перезагружалась)
        closeReg.removeEventListener("click", toggleModalReg); // Удаляю Событие нажатия на кнопку закрытия модального окна авторизации
      }
      toggleModalAuth(); // Закрываю модальное окно авторизации
      toggleModalReg(); // Открываю модальное окно регистрации
      closeReg.addEventListener("click", toggleModalReg); // Событие нажатия на кнопку закрытия модального окна авторизации
      regForm.addEventListener("submit", regIn); // Событие отправки данных формой регистрации в модальном окне
    }
  }
...
function addToCart(event) { // Функция добавления товара в корзину по нажатию на кнопку в карточке
  ...
  if (buttonAddToCart) { // Проверяю, действительно ли нажали именно на кнопку
    addAnimation(event); // Вызываю функцию отрисовки анимации добавления товара в корзину 
